...
class ConfigXmlWriter {
  ...
  String writeConfig(String fileContent) {
    ...
    final runConfRootElement = document.findAllElements('component').firstWhereOrNull((element) => element.getAttribute('name') == 'RunManager');
    ...
    flutterCommands.forEach((String command) {
      flavorsList.forEach((element) {
        addOrReplaceConf(runConfRootElement, createRunConf(config: command, flavor: element.name, dartDefines: allDartDefinesByEnv[element.flavorEnum]));
      });
    });
  ...
  void validateConfFile(XmlDocument document) {
    XmlElement? runConfRootElement = document.findAllElements('component').firstWhereOrNull((element) => element.getAttribute('name') == 'RunManager');
    if (runConfRootElement == null) {
      final projectRootElements = document.findAllElements('project');
      if (projectRootElements.isEmpty) {
        throw FormatException;
      }
      final XmlNode runManagerElement = createElementFromSkeleton(runManagerSkeleton);
      projectRootElements.first.children.add(runManagerElement);

      runConfRootElement = document.findAllElements('component').firstWhereOrNull((element) => element.getAttribute('name') == 'RunManager');
    }
  }

  XmlNode createRunConf({required String config, required String flavor, Map<String, String>? dartDefines}) {
    final XmlNode newRunConfElement = createElementFromSkeleton(runConfigSkeletonXml);
    newRunConfElement.setAttribute('name', '$config $flavor');
    final buildFlavorElement = newRunConfElement.childElements.firstWhere((element) => element.getAttribute('name') == 'buildFlavor');
    buildFlavorElement.setAttribute('value', flavor);
    final dartDefinesElement = newRunConfElement.childElements.firstWhere((element) => element.getAttribute('name') == 'additionalArgs');
    dartDefinesElement.setAttribute('value', getAdditionalArgs(command: config, flavor: flavor, dartDefines: dartDefines));
    return newRunConfElement;
  }

  XmlNode createMakeConf({required String target}) {
    final XmlNode newMakeConfElement = createElementFromSkeleton(makeConfigSkeletonXml);
  ...
  void addOrReplaceConf(XmlElement rootElement, XmlNode newConf) {
    final XmlNode? existingElement = rootElement.children.firstWhereOrNull((element) => element.getAttribute('name') == newConf.getAttribute('name'));
